...
export default function BleProvisionScreen() {
   ...
   const { state, startScan, connectToDevice, sendCredentials, reset } =
      useBleProvision();
   ...
   const prepareDeviceLink = async (espDeviceId: string) => {
      console.log('[BLE] prepareDeviceLink called with', espDeviceId);
      setIsLinking(true);
      setLinkError('');
      ...
      try {
         ...
         if (!linkRes.ok && !isBenignDeviceLinkConflict(linkRes.status, linkError)) {
            console.log('[BLE] link failed:', linkData);
            return {
               ok: false as const,
               error: getDeviceLinkFailureMessage(
                  linkRes.status,
                  linkError,
                  'Link failed',
               ),
         ...
         if (linkRes.status === 409) {
            console.log('[BLE] link already exists, continuing');
         }

         setDeviceStatus('pairedOffline');
         return { ok: true as const };
      } catch (e: unknown) {
         console.log('[BLE] prepareDeviceLink error:', e);
         return {
            ok: false as const,
   ...
   const finalizeLinkedDevice = async () => {
      console.log('[BLE] device online, hydrating auth');
   ...
   const verifyProvisionedWifi = async (espDeviceId: string) => {
      ...
      for (
         ...
         try {
            await apiFetch(`/refresh/device/${encodeURIComponent(espDeviceId)}`, {
               method: 'POST',
            }).catch(() => null);

            const response = await apiFetch(
               `/refresh/device/${encodeURIComponent(espDeviceId)}`,
               {
                  method: 'GET',
               },
            );
            ...
            if (response.status === 403) {
               lastError = 'Refresh endpoint is denying access before the device is linked.';
            } else if (!response.ok) {
   ...
   const handleSendCredentials = async () => {
      const deviceId = await sendCredentials(
         ssid.trim(),
         password,
         username.trim(),
      );
      ...
      if (deviceId) {
         const linkPreparation = await prepareDeviceLink(deviceId);
         if (!linkPreparation.ok) {
            setLinkError(linkPreparation.error);
         ...
         await finalizeLinkedDevice();
      } else {
         setLinkError(
            'Could not read device ID from display. Please reconnect and try again.',
         );
      }
   };
      ...
      >
         ...
         >
            ...
            <Text style={styles.subheading}>
               Your phone stays on home Wi-Fi the whole time. CommuteLive uses
               Bluetooth to securely send credentials to your display.
            </Text>
               ...
               <View style={styles.section}>
                  <View style={styles.scanCard}>
                     ...
                     <Text style={styles.scanText}>
                        Scanning for CommuteLive displays nearby...
                     </Text>
            ...
            {state.phase === 'connecting' && (
               <View style={styles.section}>
                  <View style={styles.scanCard}>
                     <ActivityIndicator size='large' color={colors.accent} />
                     <Text style={styles.scanText}>
                        Connecting to {state.foundDevice?.name}...
                     </Text>
                  </View>
               </View>
            )}
               ...
               <View style={styles.section}>
                  ...
                  {isVerifyingWifi && (
                     <View style={styles.progressCard}>
                        ...
                        <StatusLine
                           label='Device connecting to Wi-Fi...'
                           active={true}
                        />
                        <ActivityIndicator
                           color={colors.accent}
                  ...
                  <Pressable
                     style={[
                        ...
                        (!canSend || isBusy) && styles.primaryButtonDisabled,
                     ]}
